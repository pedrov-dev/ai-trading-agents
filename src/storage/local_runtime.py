"""Local helpers used for runtime summaries, journals, and fallback storage."""
...
class LocalInMemoryIngestionRunsRepository:
    """Small in-process repository used for local/offline runtime mode."""
...
class LocalInMemoryRawEventsRepository:
    """Minimal append-only raw event repository for local/offline mode."""
...
class LocalFileObjectStore:
    """Persist raw payloads under the local artifacts directory as gzipped JSON."""
...
class LocalEventDetectionRepository:
    """In-process detected-event repository used for local/offline runs."""
